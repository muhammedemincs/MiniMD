...
#define CH(a) printf("Reached %s %i\n",a,__LINE__);
...
int main(int argc, char** argv)
{
  ...
  for(int i = 0; i < argc; i++) {
    ...
    if((strcmp(argv[i], "-dm") == 0) || (strcmp(argv[i], "--device_map") == 0)) {
      ...
      if((str = getenv("MV2_COMM_WORLD_LOCAL_RANK")) != NULL) {
        KokkosCUDA(cudaGetDeviceCount(&dev_count);)
        local_rank = atoi(str);
        device = local_rank % ngpu;
...
#if DEVICE==1
  device_type::initialize(teams,num_threads);
#endif
  ...
  if(ghost_newton != 0 && me == 0)
    printf("# Execution on Device requires ghost_newton 0: '-gn 0'; Changing setting now.\n");
  ...
  threads.omp_me = 0;
  threads.omp_num_threads = num_threads*teams;
  threads.teams = teams;
  ...
  force->ghost_newton = ghost_newton;
  ...
  if(neighbor_size < 0 && in.datafile == NULL) {
    MMD_float neighscale = KokkosCUDA(0.48) KokkosHost(5.0 / 6.0);
  ...
  if(me == 0) {
    ...
    printf("%i %i %i %i %lf %lf %lf %lf %lf %lf %lf PERF_SUMMARY %lf\n\n\n",
           nprocs, num_threads*teams , integrate.ntimes, natoms,
