            ...
            Thermo &thermo, Integrate &integrate, Timer &timer, int screen_yaml)
{
  ...
  for(i = 0; i < atom.nlocal; i++) {
    if(atom.h_x(i, 0) < 0.0 || atom.h_x(i, 0) >= atom.box.xprd ||
        atom.h_x(i, 1) < 0.0 || atom.h_x(i, 1) >= atom.box.yprd ||
        atom.h_x(i, 2) < 0.0 || atom.h_x(i, 2) >= atom.box.zprd) nlost++;
  }
  ...
  if(me == 0) {
    ...
    if(screen_yaml) {
      ...
      fprintf(stdout, "  thread_teams: %i\n", neighbor.threads->omp_num_threads/neighbor.threads->teams);
  ...
  double mflops = 4.0 / 3.0 * 3.1415926 *
                  ...
                  23 * atom.natoms * integrate.ntimes / time_total / 1000000.0;
  ...
  if(me == 0) {
    if(screen_yaml) {
      ...
      fprintf(stdout, "    performance: %10.5e \n", 1.0 * atom.natoms * integrate.ntimes / time_total);
      fprintf(stdout, "    performance_proc: %10.5e \n", 1.0 * atom.natoms * integrate.ntimes / time_total / neighbor.threads->mpi_num_threads / neighbor.threads->omp_num_threads);
    }
  ...
  n = 0;
  Kokkos::deep_copy(neighbor.h_numneigh, neighbor.numneigh);

  for(i = 0; i < atom.nlocal; i++) n += neighbor.h_numneigh[i];
