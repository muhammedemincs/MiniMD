...
void read_lammps_atoms(Atom &atom, t_x_array_host x)
{
  int i;
...
void read_lammps_velocities(Atom &atom, t_v_array_host v)
{
  int i;
...
int read_lammps_data(Atom &atom, Comm &comm, Neighbor &neighbor, Integrate &integrate, Thermo &thermo, char* file, int units)
{
  ...
  if(neighbor.nbinx < 0) {
    ...
    neighbor.nbinx = KokkosCUDA(0.44 *)atom.box.xprd / neigh_bin_size;
    neighbor.nbiny = KokkosCUDA(0.44 *)atom.box.yprd / neigh_bin_size;
    neighbor.nbinz = KokkosCUDA(0.44 *)atom.box.zprd / neigh_bin_size;
  }
  ...
  t_x_array_host x("tmpx", atom.natoms);
  t_v_array_host v("tmpv", atom.natoms);
